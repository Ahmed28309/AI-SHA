#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# FastDDS environment setup for AI-SHA 4-device mesh
#
...
#
#   # In ~/.bashrc on RPi 4b:
#   source /home/pi/robot_ws/config/fastdds_env.sh rpi4
#
#   # On dev workstation:
...
if [ -z "$DEVICE" ]; then
    echo "[fastdds_env] ERROR: specify device name: jetson | rpi | rpi4 | ws"
